...
// RGB led colorSketch uses 31632 bytes (98%) of program storage space. Maximum is 32256 bytes.
...
void tokenTree() {
  ...
  switch (TOKEN_BUFFER[current]) {
    ...
    case t_SET:  // Deals with all possiblities from a set token
      ...
      switch (TOKEN_BUFFER[current]) {
        ...
        case t_BLINK:  // Deals with all possiblities from a set token
          ...
          setBlink((int)(((unsigned)TOKEN_BUFFER[current + 1] << 8) | TOKEN_BUFFER[current]));  // gets origional number back
          break;

        case t_COLOR:
          current += (3 * 3 + 1);  // 3 * num input numbers + 1
          if (EOL(current) == 1) {
            return;
          }
          setColor();
    ...
    case t_ADD:
      if (EOL(current + (3 * 2 + 1)) == 1) {  // 3 * num input numbers + 1
        return;
      }
...
void controlLeds() {
  ...
  leds[1].setRGB(0, 0, 255);  // controls power led
...
/* The below code handles blink input */
...
/* The below code handles add input */
int check_if_add(int TOKENCOUNTER, int i) {
  if ((int)TOKEN_BUFFER[TOKENCOUNTER - 1] == (int)t_ADD) {
    TOKENCOUNTER = getNumbers(TOKENCOUNTER, i, 2);
  }
  return TOKENCOUNTER;
}


/* The below code handles set color input */
...
int setTempHumid() {
  if (millis() - LAST_TIME_RECORD >= 5000) {    // Reads every 5 seconds
    ...
    lcd.clear();  // Clears LCD for update     WILL NEED TO CHANGE NEXT PROJECT
    RECORD_COUNTER++;
  }
}
...
void setColor() {
  ...
  if (red <= 255 && green <= 255 && blue <= 255 && red >= 0 && green >= 0 && blue >= 0) {
    ...
    Udp.print(F("ERROR: OUT OF BOUNDS (0-255"));
  }
}
...
/* The code below sets the bi-led to blink red/green */
void ledBlinkRG() {
  LED_CONTROL = 187;  // 10 11 10 11
  Serial.println(F("LED blink set to r/g"));
  Udp.println(F("LED blink set to r/g"));
}


/* The code below sets the bi-led to blink red or green */
void ledBlink() {
  LED_CONTROL = LED_CONTROL ^ 136;  // swaps 10 00 10 00
  Serial.println(F("LED blink toggle"));
  Udp.println(F("LED blink toggle"));
}


/* The code below sets the bi-led to red */
void ledRed() {
  LED_CONTROL = LED_CONTROL & 170;  // sets all color bits to 0 (170 = 10101010)
  Serial.println(F("LED red"));
  Udp.println(F("LED red"));
}


/* The code below sets the bi-led to green */
void ledGreen() {
  LED_CONTROL = LED_CONTROL | 85;  // sets all color bits to 0 (85 = 01010101)
  Serial.println(F("LED green"));
  Udp.println(F("LED green"));
}


/* The code below sets the bi-led on */
void ledOn() {
  LED_CONTROL = LED_CONTROL | 170;  // sets all power bits to 1 (170 = 10101010)
  Serial.println(F("LED on"));
  Udp.println(F("LED on"));
}


/* The code below sets the bi-led off */
void ledOff() {
  LED_CONTROL = LED_CONTROL & 85;  // 01010101
  Serial.println(F("LED off"));
  Udp.println(F("LED off"));
}


/* The code below sets the blink rate */
void setBlink(int rec) {
  BLINK_RATE = rec;
  Serial.print(F("Blink rate set to: "));
  Serial.println(rec);
  Udp.print(F("Blink rate set to: "));
  Udp.println(rec);
}
...
/* The code below sets the d13 to blink */
void d13Blink() {
  D13_CONTROL = 170;
  Serial.println(F("D13 blink toggled"));
  Udp.println(F("D13 blink toggled"));
}

/* The code below sets the d13 on */
void d13On() {
  D13_CONTROL = 255;
  Serial.println(F("D13 on"));
  Udp.println(F("D13 on"));
}


/* The code below sets the d13 off */
void d13Off() {
  D13_CONTROL = 0;
  Serial.println(F("D13 off"));
  Udp.println(F("D13 off"));
}


/* The code below sets the rgb to blink */
void rgbBlink() {
  RGB_CONTROL = 170;
  Serial.println(F("RGB blink toggled"));
  Udp.println(F("RGB blink toggled"));
}


/* The code below sets the rgb on */
void rgbOn() {
  RGB_CONTROL = 255;
  Serial.println(F("RGB on"));
  Udp.println(F("RGB on"));
}


/* The code below sets the rgb off */
void rgbOff() {
  RGB_CONTROL = 0;
  Serial.println(F("RGB off"));
  Udp.println(F("RGB off"));
}
...
void print_help() {
  ...
  Udp.println(F("led <on, off, blink <NONE, rg>, green, red>"));
  Udp.println(F("d13 <on, off, blink>"));
  Udp.println(F("RGB <on, off, blink>"));
  Udp.println(F("set blink <number 0-65535>"));
  Udp.println(F("set color <RRR GGG BBB"));
  Udp.println(F("status leds"));
...
/* The following code prints the status of the leds */
void print_status() {
  Serial.println(F("Current status:"));
  Udp.println(F("Current status:"));
  ...
  Udp.print(F("BLINK RATE: "));
  Udp.println(BLINK_RATE);


  Serial.print(F("Connection LED: "));
  Udp.print(F("Connection LED: "));
  ...
  Serial.print(F("Temperature LED: "));
  Udp.print(F("Temperature LED: "));

  int temperature = convert_c_f(TEMPERATURE);
  if (temperature <= 60) {
    Serial.println(F("Purple"));
    Udp.println(F("Connection LED"));
...
/* The code below adds a and b as ints */
...
void alarm() {
  ...
  if (temperature <= 60 && LAST_SENT_ALARM != 1) {
    Udp.beginPacket(Udp.remoteIP(), Udp.remotePort());
    leds[3].setRGB(230, 230, 250);  // Temp 60 and below
    ...
    leds[3].setRGB(0, 0, 255);  // Temp 61-70
    ...
    leds[3].setRGB(0, 255, 0);  // Temp 71-80
    ...
    leds[3].setRGB(255, 165, 0);  // Temp 81-90
